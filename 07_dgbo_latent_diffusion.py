...
def guided_sample_latent_z0_norm_batch(
    ...
    """Batch guided DDPM sampling. Internal samples are not oracle calls."""
    ...
    for start in range(0, n_samples, batch_size):
        ...
        for t in reversed(range(T)):
            ...
            if guidance_scale > 0.0 and do_guide:
                ...
                x_scaled = vae.decode(z0_hat_real)
                x_rad = x_scaled * np.pi
                X_np = x_rad.detach().cpu().numpy().astype(np.float64)
                grad_rows = []
                for xb in X_np:
                    if weight == "pi":
                        grad_x = grad_log_weight_PI(gp, xb, best_y=best_y, xi=xi)
                    elif weight == "ei":
                        grad_x = grad_log_weight_EI(gp, xb, best_y=best_y, xi=xi)
                    else:
                        raise ValueError("weight must be 'pi' or 'ei'")
                    grad_rows.append(tau_guidance * grad_x)

                grad_x_torch = torch.tensor(np.stack(grad_rows), device=device, dtype=torch.float32)
                scalar = (x_rad * grad_x_torch).sum()
                if z0_hat_real.grad is not None:
                    z0_hat_real.grad.zero_()
                scalar.backward()
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        default=None,
        help="Where to save plots/traces. Supports {seed} and {method}; default is results/toy_runs/dgbo_latent_diffusion/<config>/seed_{seed}.",
    )
    ...
    # plotting dirs. Seed-aware by default so repeated runs do not overwrite each other.
    default_tag = f"tau{args.tau_guidance}_gs{args.guidance_scale}_clip{args.clip_guidance}_cand{args.n_cand}"
    plotroot_template = args.plotroot or os.path.join(
        "results", "toy_runs", "dgbo_latent_diffusion", default_tag, "seed_{seed}"
    )
    plot_root = plotroot_template.format(seed=args.seed, method="dgbo_latent_diffusion")
