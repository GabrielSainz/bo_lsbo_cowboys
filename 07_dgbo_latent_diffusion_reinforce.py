# 07_dgbo_latent_diffusion_reinforce.py
...
# - Guidance uses a Monte Carlo REINFORCE estimator for ∇_{z_t} J_n(h_theta(z0_hat))
#
# Usage example:
# tau = 5.0
...
# mkdir -p "$plotroot"
# python 07_dgbo_latent_diffusion_reinforce.py --outdir toy_circle_data --n_steps 60 --n_init 24 --weight pi --xi 0.06 --guide_every 1 --n_cand 1 --tau_guidance 5 --clip_guidance 10 --reinforce_k 16 --reinforce_sigma 0.08 > toy_circle_data/plots_dgbo_tau5.0/log.log 2>&1
#
# Notes:
...
def reinforce_latent_grad(
    ...
    """
    Monte Carlo REINFORCE estimate of grad_z E_{x~p(x|z)}[log w(x)].

    In this toy continuous decoder, p(x|z) is modeled as a factorized Gaussian:
      x_scaled ~ N(mu(z), sigma^2 I), with mu(z)=vae.decode(z) in [-1,1]^L.
    """
    ...
    z_var = z_real.detach().clone().requires_grad_(True)  # (1,2)

    mu_scaled = vae.decode(z_var)             # (1,L)
    mu_rep = mu_scaled.expand(k, -1)          # (K,L)
    sigma = float(reinforce_sigma)

    # Score-function estimator: sample x and treat sampled x as constant in ∇_z log p(x|z).
    ...
    if baseline_mode == "mean":
        baseline = float(np.mean(logw))
    elif baseline_mode == "median":
        baseline = float(np.median(logw))
    ...
    adv = logw - baseline
    if normalize_adv:
        adv = adv / max(float(np.std(adv)), 1e-6)
    if adv_clip > 0.0:
...
def guided_sample_latent_z0_norm(
    ...
    for t in reversed(range(T)):
        ...
        if guidance_scale > 0.0 and do_guide:
            ...
            grad_z0_real, rf_info = reinforce_latent_grad(
                vae=vae,
                gp=gp,
                z_real=z0_hat_real,
                best_y=best_y,
            ...
            gz0_norm_raw = float(torch.norm(grad_z0_real).item())
            grad_z0_real = tau_guidance * grad_z0_real
            gz0_norm_tau = float(torch.norm(grad_z0_real).item())
            ...
            if should_print:
                print(
                    ...
                    f"K={rf_info['k']} base={rf_info['baseline']:,.3f} "
                    f"logw(mu+/-sd)={rf_info['logw_mean']:,.3f}+/-{rf_info['logw_std']:,.3f} "
                    f"||gz0|| raw={gz0_norm_raw:,.4f} tau={gz0_norm_tau:,.4f} "
...
def main():
    ...
    ap.add_argument("--reinforce_k", type=int, default=16)
    ap.add_argument("--reinforce_sigma", type=float, default=0.08)
    ap.add_argument("--reinforce_baseline", type=str, default="mean", choices=["mean", "median", "zero"])
    ap.add_argument("--reinforce_logw_clip_low", type=float, default=-30.0)
    ap.add_argument("--reinforce_logw_clip_high", type=float, default=0.0)
    ap.add_argument("--reinforce_adv_clip", type=float, default=20.0)
    ap.add_argument("--reinforce_normalize_adv", dest="reinforce_normalize_adv", action="store_true")
    ap.add_argument("--no_reinforce_normalize_adv", dest="reinforce_normalize_adv", action="store_false")
    ...
    ap.set_defaults(reinforce_normalize_adv=True)
    ...
    if args.diagnostics:
        diagnostics = ToyDiagnosticsLogger(
            method="dgbo_latent_diffusion_reinforce",
            seed=args.seed,
    ...
    print(
        "[REINFORCE] "
        f"K={args.reinforce_k} sigma={args.reinforce_sigma:.4f} "
        f"baseline={args.reinforce_baseline} "
        f"logw_clip=[{args.reinforce_logw_clip_low:.2f},{args.reinforce_logw_clip_high:.2f}] "
        f"adv_clip={args.reinforce_adv_clip:.2f} norm_adv={args.reinforce_normalize_adv}"
    )

    assert X.shape[1] == L, f"Dataset L={X.shape[1]} but VAE expects L={L}"

    # plotting dirs
    #plot_root = os.path.join(outdir, "03_dgbo_runs_reinforce")
    plot_root = args.plotroot if args.plotroot is not None else os.path.join(outdir, "03_dgbo_runs_reinforce")
