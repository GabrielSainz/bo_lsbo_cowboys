...
def guided_sample_latent_z0_norm_batch(
    ...
    """Batch guided DDPM sampling. Internal samples are proposal evaluations only."""
        ...
        for t in reversed(range(T)):
            ...
            if guidance_scale > 0.0 and do_guide:
                ...
                if guide_mode == "real":
                    ...
                    v_pred.backward()
                    grad_z0_real = z0_hat_real.grad.detach()
                    grad_z_t = tau_guidance * (z_std_t / torch.clamp(sqrt_ab, min=1e-12)) * grad_z0_real

                    if verbose and distill_compare_every > 0 and gp is not None and (t % distill_compare_every == 0):
                        with torch.no_grad():
                            x_dbg = (vae.decode(z0_hat_real[:1].detach()) * np.pi).detach().cpu().numpy().reshape(-1).astype(np.float64)
                            if weight == "pi":
                                logw_true = gp_acquisition(gp, x_dbg, best_y=best_y, acq="pi", xi=xi)
                            else:
                                ei_true = gp_acquisition(gp, x_dbg, best_y=best_y, acq="ei", xi=xi)
                                logw_true = float(np.log(max(ei_true, 1e-12)))
                            print(f"[DISTILL] t={t:4d} v_pred={float(v_pred.detach().item() / B):,.4f} logw_true={logw_true:,.4f}")
                else:
                    raise ValueError("guide_mode must be 'real' or 'distill'")
...
def sample_unconditional_latents(
    ...
    z0n, _ = guided_sample_latent_z0_norm_batch(
        eps_model=eps_model,
        vae=vae,
        gp=None,
        betas=betas,
    ...
    return (z0n * z_std + z_mean).astype(np.float64)
...
def main():
    ...
    for it in range(1, args.n_steps + 1):
        ...
        if args.guide_mode == "distill":
            ...
            if plot_this_iter and Z_grid is not None:
                with torch.no_grad():
